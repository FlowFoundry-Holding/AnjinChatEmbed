...
const ANJIN_API_KEY = process.env.ANJIN_API_KEY;
...
if (!ANJIN_API_KEY) {
  console.error('ANJIN_API_KEY is not set in environment variables');
  process.exit(1);
}
...
const validateApiKey = (req, res, next) => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ') && authHeader.split(' ')[1] === ANJIN_API_KEY) {
    return next();
  }
